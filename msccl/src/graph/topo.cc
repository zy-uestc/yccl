...
ncclResult_t ncclTopoGetSystemPCIE(struct ncclComm* comm, struct ncclTopoSystem** systemPCIE) {
  ncclResult_t ret = ncclSuccess;
  struct ncclXml* xml;
  char* mem = NULL;
  int* localRanks = NULL;
  ...
  const char* xmlTopoFile = ncclGetEnv("NCCL_TOPO_FILE_PCIE");
  if (xmlTopoFile) {
  ...
  NCCLCHECKGOTO(ncclTopoFillGpuPCIE(xml, busId, &node), ret, fail);
  if (node) {
  ...
  xmlTopoFile = ncclGetEnv("NCCL_TOPO_DUMP_FILE_PCIE");
  ...
  NCCLCHECKGOTO(ncclTopoGetSystemFromXml(xml, systemPCIE, comm->peerInfo[comm->rank].hostHash), ret, fail);
exit:
