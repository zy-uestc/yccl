...
// 重命名函数，避免与头文件中的数组冲突
const char* getTopoNodeTypeStr(int type) {
  switch(type) {
    case GPU: return "GPU";
    case NET: return "NET";
    case CPU: return "CPU";
...
// 重命名路径类型转换函数
const char* getPathTypeStr(int type) {
  switch(type) {
    // Local (myself)
...
// 新增的路径打印函数
static void printPathList(struct ncclTopoNode* node, struct ncclTopoLinkList* pathList) {
    INFO(NCCL_INIT, "  -> Dest Type: %s\n", getTopoNodeTypeStr(node->type));
...
void printIntraPaths(struct ncclTopoGraph* graph) {
 INFO(NCCL_INIT,"Intra-node paths (nChannels=%d):\n", graph->nChannels);
  for (int c = 0; c < graph->nChannels; c++) {
    INFO(NCCL_INIT,"  Channel %d: ", c);
    for (int i = 0; i < NCCL_TOPO_MAX_NODES; i++) {
      int nodeIdx = graph->intra[c * NCCL_TOPO_MAX_NODES + i];
      if (nodeIdx == -1) break; // 路径结束标记
      INFO(NCCL_INIT,"%d ", nodeIdx);
    }
    INFO(NCCL_INIT,"\n");
  }
}

void printInterPaths(struct ncclTopoGraph* graph) {
  INFO(NCCL_INIT,"Inter-node paths (nChannels=%d):\n", graph->nChannels);
  for (int c = 0; c < graph->nChannels; c++) {
    // inter 数组每通道占 2 个元素（发送/接收方向）
    int64_t sendTarget = graph->inter[c * 2];
    int64_t recvTarget = graph->inter[c * 2 + 1];
    INFO(NCCL_INIT,"  Channel %d: Send→0x%lx, Recv←0x%lx\n", c, sendTarget, recvTarget);
  }
}

// 函数声明 (需放在头文件或函数调用前)
static ncclResult_t mergePCIERingGraph(struct ncclComm* comm, struct ncclTopoGraph* ringGraph) {
  struct ncclTopoGraph ringGraphPCIE;
  memset(&ringGraphPCIE, 0, sizeof(struct ncclTopoGraph));
  ringGraphPCIE.id = 0;
  ringGraphPCIE.pattern = NCCL_TOPO_PATTERN_RING;
  ringGraphPCIE.minChannels = 1;
  ringGraphPCIE.maxChannels = MAXCHANNELS / 8;

  // 计算PCIE拓扑
  NCCLCHECK(ncclTopoCompute(comm->topoPCIE, &ringGraphPCIE));
  
  // 合并通道
  const int originalChannels = ringGraph->nChannels;
  int newChannels = ringGraphPCIE.nChannels;
  const int totalChannels = originalChannels + newChannels;

  if (totalChannels > MAXCHANNELS) {
    newChannels = MAXCHANNELS - originalChannels;
    if (newChannels <= 0) return ncclSuccess; // 无需合并
  }

  // 检查数组边界
  if ((originalChannels + newChannels)*NCCL_TOPO_MAX_NODES > sizeof(ringGraph->intra)/sizeof(int) ||
      (originalChannels + newChannels)*2 > sizeof(ringGraph->inter)/sizeof(int64_t)) {
    WARN("Channel data overflow during PCIe merge");
    return ncclInternalError;
  }
  ...
  // 合并 intra 数组
  {
      ...
      const int intraTotalElements = MAXCHANNELS * NCCL_TOPO_MAX_NODES;

      // 1. 检查越界
      if (destStart + srcValidElements > intraTotalElements) {
          WARN("intra 合并失败：需要 %d 元素，但数组容量为 %d", 
              destStart + srcValidElements, intraTotalElements);
          return ncclInternalError;
      }

      // 2. 复制有效元素
      memcpy(
          &ringGraph->intra[destStart],
          ringGraphPCIE.intra,
  ...
  // 合并 inter 数组
  {
      ...
      // 1. 检查越界
      if (destStart + srcValidElements > interTotalElements) {
          WARN("inter 合并失败：需要 %d 元素，但数组容量为 %d", 
              destStart + srcValidElements, interTotalElements);
          return ncclInternalError;
      }

      // 2. 复制有效元素
      memcpy(
          &ringGraph->inter[destStart],
          ringGraphPCIE.inter,
  ...
  // 更新通道数
  ringGraph->nChannels = originalChannels + newChannels;
  // 更新参数
  // printf("=================ringGraph->nChannels %d,newChannels %d=============",ringGraph->nChannels,newChannels);
  // ringGraph->nChannels += newChannels;
  ringGraph->bwIntra = std::max(ringGraph->bwIntra, ringGraphPCIE.bwIntra);
  ringGraph->bwInter = std::max(ringGraph->bwInter, ringGraphPCIE.bwInter);
...
static ncclResult_t initTransportsRank(struct ncclComm* comm, struct ncclComm* parent, uint64_t timers[TIMERS_INIT_COUNT]) {
  ...
  // PCIE
  NCCLCHECKGOTO(ncclTopoGetSystemPCIE(comm, &comm->topoPCIE), ret, fail);
  // Compute paths between GPUs and NICs
  NCCLCHECKGOTO(ncclTopoComputePaths(comm->topoPCIE, comm), ret, fail);
  // Remove inaccessible GPUs and unused NICs
  NCCLCHECKGOTO(ncclTopoTrimSystem(comm->topoPCIE, comm), ret, fail);
  // Recompute paths after trimming
  NCCLCHECKGOTO(ncclTopoComputePaths(comm->topoPCIE, comm), ret, fail);
  // Init search
  NCCLCHECKGOTO(ncclTopoSearchInit(comm->topoPCIE), ret, fail);
