  ...
  __device__ __forceinline__ void genericOp(
    ...
    if (tid < nworkers && offset < nelem && !isNetOffload) {
      ...
      do {
        ...
        // if (tid ==0 && (Send||DirectSend)&&  ncclShmem.channelId ==0&&  ncclShmem.comm.rank ==0){
        //   for (int i =0;i<NCCL_MAX_ARITY;i++){
        //     struct ncclConnInfo* sendConn = ncclShmem.groups[group].sendConns[i];
        //     if (sendConn == NULL || sendConn->head == NULL || sendConn->tail == NULL) continue;
        //     // 确保 head/tail 是设备指针
        //     uint64_t head_val = *sendConn->head;
        //     uint64_t tail_val = *sendConn->tail;
        //     // slice < SlicePerChunk && offset < nelem
        //     printf("[time %llu] After waitPeer [channel %d] [stepSize %d] [nelem %d] [workSize %d] [buffSizes %d] slice:%d, offset:%d, head=%llu, tail=%llu, step:%llu\n",
        //           clock64(),ncclShmem.channelId,stepSize, nelem,workSize,ncclShmem.comm.buffSizes[NCCL_PROTO_SIMPLE],slice,offset,(unsigned long long)head_val, (unsigned long long)tail_val,(unsigned long long)step);
    ...
    //     uint64_t tail_val = *sendConn->tail;
    //     // slice < SlicePerChunk && offset < nelem
    //     printf("[time %llu] lice < SlicePerChunk [channel %d] [SlicePerChunk %d] [nelem %d] [sliceSize %d] [buffSizes %d] slice:%d, offset:%d, head=%llu, tail=%llu,step=%llu, slice < SlicePerChunk:%d\n",
    //           clock64(),ncclShmem.channelId,SlicePerChunk, nelem,sliceSize,ncclShmem.comm.buffSizes[NCCL_PROTO_SIMPLE],slice,offset,(unsigned long long)head_val, (unsigned long long)tail_val,(unsigned long long)step,slice < SlicePerChunk);
