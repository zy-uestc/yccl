...
cd $MAIN_DIR/nccl-tests && make -j MPI=1 MPI_HOME=/usr/lib/x86_64-linux-gnu/openmpi NPKIT_FLAGS="${NPKIT_FLAGS}" CUDA_HOME=/usr/local/cuda NCCL_HOME=$MAIN_DIR/nccl/build/
# env
...
for ((j=0;j<1;j++)) do
    ...
    -x NCCL_PROTO=Simple \
    -x NCCL_DEBUG=info \
    $MAIN_DIR/nccl-tests/build/alltoall_perf \
    -b 1G -e 1G -f 2 -g 1 -c 1 -n 1 -w 5 -G 0 -z 0 || true
